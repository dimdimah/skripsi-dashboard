  ...
  return (
    <div className="page-container space-y-8 pb-8">
      ...
      <div className="space-y-2 animate-fade-in-up">
        ...
        <p className="text-slate-600">Temukan peluang karir terbaik untuk alumni.</p>
      </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 animate-fade-in-up" style={{ animationDelay: '0.12s' }}>
          {filteredJobs.length > 0 ? (
            filteredJobs.map((job, index) => (
              <div
                key={job.id}
                className="group relative rounded-lg border border-slate-200 bg-white p-6 shadow-sm transition-all duration-200 hover:shadow-sm hover:-translate-y-0.5 cursor-pointer"
                ...
                <div className="relative space-y-4">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <h3 className="font-sans text-base font-semibold text-slate-900 truncate">{job.title}</h3>
                  ...
                  {(job.skills && job.skills.length > 0) && (
                    <div className="flex flex-wrap gap-1.5">
                      {job.skills.map((skill) => (
                        <span key={skill} className="inline-flex items-center rounded-md border border-slate-200/60 bg-slate-50/60 px-2.5 py-1 text-[11px] font-medium font-mono text-slate-600">
                          {skill}
                        </span>
                      ))}
                    </div>
      ...
      {/* Modal Detail */}
      {selectedJob && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={() => setSelectedJob(null)}>
          <div className="w-full max-w-lg mx-4 rounded-lg border border-slate-200 bg-white p-6 shadow-sm animate-fade-in-up" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between mb-4">
              <div>
                <Badge variant={selectedJob.type === 'Full-time' ? 'default' : selectedJob.type === 'Internship' ? 'warning' : 'secondary'}>
                  {selectedJob.type}
                </Badge>
                <h2 className="font-sans text-xl font-semibold text-slate-900 mt-2">{selectedJob.title}</h2>
                <div className="flex items-center gap-2 mt-1">
              ...
              <button onClick={() => setSelectedJob(null)} className="text-slate-400 hover:text-slate-600 text-xl leading-none">&times;</button>
            </div>
